...
interface InvertedBoxDashRunnerProps {
...
export default function InvertedBoxDashRunner({
  Asset,
  assets,
  backgroundColor = '#fff',
}: InvertedBoxDashRunnerProps) {
  ...
  const spawnCritters = useCallback(() => {
    ...
    setCritters((prev) => {
      ...
      const boxTargetPadding = Math.max(6, Math.round(size * 0.1));

      for (let i = 0; i < count; i += 1) {
        const chosenAsset = availableCritterAssets[Math.floor(Math.random() * availableCritterAssets.length)];
        const startX = clamp(Math.random() * (SCREEN_WIDTH - size), -targetInset, SCREEN_WIDTH + targetInset);
        const startY = -targetInset - Math.random() * (size * 1.8);

        const targetX = clamp(
          boxX + boxTargetPadding + Math.random() * Math.max(1, boxSize - size - boxTargetPadding * 2),
          boxX,
          boxX + boxSize - size
        );
        const targetY = clamp(
          boxY + boxSize * 0.45 + (Math.random() * 2 - 1) * (size * 0.25),
          boxY + boxTargetPadding,
          boxY + boxSize - size - boxTargetPadding
        );
