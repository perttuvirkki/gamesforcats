...
interface BoxEscapeRunnerProps {
...
function cornerTarget(corner: Corner, size: number) {
  const o = size * 2.2;
  switch (corner) {
    case 'tl':
      return { x: -o, y: -o };
    case 'tr':
      return { x: SCREEN_WIDTH + o, y: -o };
    case 'bl':
      return { x: -o, y: SCREEN_HEIGHT + o };
    case 'br':
      return { x: SCREEN_WIDTH + o, y: SCREEN_HEIGHT + o };
  }

  return { x: -o, y: -o };
}

export default function BoxEscapeRunner({ Asset, assets, backgroundColor = '#fff' }: BoxEscapeRunnerProps) {
  ...
  const boxSize = Math.min(200, Math.max(100, SCREEN_WIDTH * 0.32));
  const boxX = (SCREEN_WIDTH - boxSize) / 2;
  const boxY = (SCREEN_HEIGHT - boxSize) / 2;
  ...
  const spawnCritters = useCallback(() => {
    ...
    setCritters((prev) => {
      ...
      for (let i = 0; i < count; i += 1) {
        const targetCorner = pickCorner();
        const target = cornerTarget(targetCorner, size);
        ...
        next.push({
          ...
          startX: boxX + boxSize / 2 - size / 2 + (Math.random() * 2 - 1) * (size * 0.2),
          startY: boxY + boxSize * 0.62 - size / 2 + (Math.random() * 2 - 1) * (size * 0.15),
          targetX: target.x,
          targetY: target.y,
  ...
  }, [availableCritterAssets, boxSize, boxX, boxY, size, speed]);
